...
vector<ofVboMesh> ofxFontThreeDee::extrude_mesh_from_text(string word, float extrusion_depth) {

	// contains all of the paths of the current word
	vector <ofPath> word_paths = this->getStringAsPoints(word, 0, 0);
	...
	for (int i = 0; i < word_paths.size(); i++) {
		...
		for (int j = 0; j < lines.size(); j++) {
			...
			ofVboMesh side;
			vector <glm::vec3> points = lines.at(j).getVertices();
			int k = 0;
